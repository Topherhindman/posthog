...
export class BatchPipelineBuilder<TInput, TOutput, CInput, COutput = CInput, R extends string = never> {
    ...
    /**
     * Add a batch step with per-event retry, overflow redirect, DLQ routing,
     * and optional circuit breaking. The step returns per-event success/failure.
     * The wrapper retries only the failed events, DLQs non-retriable failures,
     * and overflows retriable failures when some events in the batch succeeded.
     * If all events fail, throws CircuitOpenError for the consumer to handle.
     */
