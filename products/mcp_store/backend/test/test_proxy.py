...
class TestMCPProxyToolApproval(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    @patch("products.mcp_store.backend.proxy.httpx.Client")
    def test_mixed_batch_rejects_whole_request(self, mock_client_cls):
        """Batches that mix approved + needs-approval must be rejected atomically."""
        installation = self._installation()
        ...
        body = response.json()
        assert isinstance(body, list)
        # Every item rejected; upstream never called.
        for entry in body:
            assert entry["error"]["code"] == -32001
        mock_client_cls.assert_not_called()
