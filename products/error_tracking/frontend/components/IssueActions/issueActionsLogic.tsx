...
import { captureMergeOverrides, captureOverridesForIssues } from '../../logics/captureOverridesForIssues'
    ...
    listeners(({ actions }) => {
        async function runMutation(
            ...
            try {
                await cb()
                if (onSuccess) {
                    // Populate the fingerprint override cache BEFORE mutationSuccess
                    // so the blocking reload it triggers already carries overrides.
                    await onSuccess()
                }
        ...
        return {
            mergeIssues: async ({ ids }) => {
                ...
                if (firstId && otherIds.length > 0) {
                    await runMutation(
                        ...
                        async () => captureMergeOverrides(firstId, otherIds)
                    )
                }
            },
            splitIssue: async ({ id, fingerprints }) => {
                // Splits fan out to newly-created issues whose ids we don't know
                // up front, so skip the override cache here — existing blocking
            ...
            resolveIssues: async ({ ids }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues(ids, { status: 'resolved' })
                )
            ...
            suppressIssues: async ({ ids }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues(ids, { status: 'suppressed' })
                )
            },
            activateIssues: async ({ ids }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues(ids, { status: 'active' })
                )
            },
            assignIssues: async ({ ids, assignee }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues(ids, { assignee })
                )
            },
            updateIssueAssignee: async ({ id, assignee }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues([id], { assignee })
                )
            },
            updateIssueStatus: async ({ id, status }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues([id], { status })
                )
            },
            updateIssueName: async ({ id, name }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues([id], { name })
                )
            },
            updateIssueDescription: async ({ id, description }) => {
                await runMutation(
                    ...
                    async () => captureOverridesForIssues([id], { description })
                )
            },
