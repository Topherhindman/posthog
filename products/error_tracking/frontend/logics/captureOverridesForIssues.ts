...
import { fingerprintIssueStateCacheLogic } from './fingerprintIssueStateCacheLogic'
 ...
 * and stash the resulting rows into the frontend override cache. The cache is
 ...
 */
export async function captureOverridesForIssues(issueIds: string[], delta: IssueStateDelta): Promise<void> {
    try {
        ...
        for (const id of uniqueIds) {
            ...
            for (const fp of fingerprints) {
                rows.push(buildOverrideRow(fp, id, merged, version))
            }
        }

        if (rows.length > 0) {
            fingerprintIssueStateCacheLogic.findMounted()?.actions.addRows(rows) ??
                fingerprintIssueStateCacheLogic.actions.addRows(rows)
        }
    } catch {
        // swallow — override cache is best-effort
    }
}
 ...
 */
export async function captureMergeOverrides(primaryId: string, sourceIds: string[]): Promise<void> {
    try {
        ...
        for (const id of allIds) {
            const fingerprints = fingerprintMap[id] ?? []
            for (const fp of fingerprints) {
                rows.push(buildOverrideRow(fp, primaryId, primaryState, version))
            }
        }
