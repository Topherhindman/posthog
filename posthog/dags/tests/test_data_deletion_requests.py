...
def test_execute_event_deletion_deferred_queues_into_adhoc_table(cluster: ClickhouseCluster):
    ...
    other_event_name = [(DEFERRED_TEAM_ID, "$identify", uuid4(), now - timedelta(hours=i)) for i in range(10)]
    other_team = [(DEFERRED_TEAM_ID + 1, "$pageview", uuid4(), now - timedelta(hours=i)) for i in range(5)]

    cluster.any_host(partial(_insert_events, matching_events + other_event_name + other_team)).result()

    pre_count = cluster.any_host(partial(_count_events_by_name, DEFERRED_TEAM_ID, "$pageview")).result()
    assert pre_count == 25
    ...
    # Events are NOT deleted by the job itself in deferred mode.
    post_count = cluster.any_host(partial(_count_events_by_name, DEFERRED_TEAM_ID, "$pageview")).result()
    assert post_count == 25

    # Matching UUIDs land in adhoc_events_deletion; non-matching ones do not.
    queued = cluster.any_host(partial(_adhoc_pending_uuids, DEFERRED_TEAM_ID)).result()
    assert queued == set(matching_uuids)

    # Cross-team rows are not queued under DEFERRED_TEAM_ID.
    queued_other = cluster.any_host(partial(_adhoc_pending_uuids, DEFERRED_TEAM_ID + 1)).result()
    assert not queued_other
...
def test_full_job_event_deletion_deferred(cluster: ClickhouseCluster):
    ...
    assert cluster.any_host(partial(_count_events_by_name, DEFERRED_TEAM_ID, "$identify")).result() == 10
...
@pytest.mark.django_db
def test_verify_queued_deletion_requests_promotes_when_events_gone(cluster: ClickhouseCluster):
    ...
    # No matching events in ClickHouse — count should be zero.
    assert _count_remaining_matching_events(request) == 0

    # Exercise the sensor body directly (skipping the @run_status_sensor decorator,
    # which we can't fire via the dagster daemon in a unit test).
    from django.utils import timezone

    queued_requests = DataDeletionRequest.objects.filter(status=RequestStatus.QUEUED)
    for queued in queued_requests:
        if _count_remaining_matching_events(queued) == 0:
            DataDeletionRequest.objects.filter(pk=queued.pk, status=RequestStatus.QUEUED).update(
...
@pytest.mark.django_db
def test_verify_queued_deletion_requests_leaves_queued_when_events_remain(cluster: ClickhouseCluster):
    from posthog.dags.data_deletion_requests import _count_remaining_matching_events
    ...
    remaining_event = [(DEFERRED_TEAM_ID, "$pageview", uuid4(), now - timedelta(hours=1))]
    cluster.any_host(partial(_insert_events, remaining_event)).result()
    ...
    assert _count_remaining_matching_events(request) >= 1

    # Simulated sensor body: since count > 0, no promotion happens.
    queued_requests = DataDeletionRequest.objects.filter(status=RequestStatus.QUEUED)
    for queued in queued_requests:
        if _count_remaining_matching_events(queued) == 0:
            from django.utils import timezone
