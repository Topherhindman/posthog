...
def _attach_messages(request) -> None:
    """The admin view emits messages.* calls; the test RequestFactory needs a storage backend."""
    request.session = {}
    request._messages = FallbackStorage(request)


def _fake_reverse(name, args=None, kwargs=None):
    """Bypass NoReverseMatch in tests where the admin URL conf isn't mounted (ADMIN_PORTAL_ENABLED)."""
    if args:
...
class TestDataDeletionRequestAdminApprovalFlow(BaseTest):
    ...
    def test_approve_view_get_renders_picker_for_event_removal(self):
        ...
        self.assertEqual(response.template_name, "admin/posthog/datadeletionrequest/approve.html")
        context = response.context_data
        self.assertTrue(context["supports_deferred"])
        self.assertEqual(context["default_execution_mode"], ExecutionMode.IMMEDIATE)
        choice_values = [value for value, _ in context["execution_mode_choices"]]
        self.assertIn(ExecutionMode.IMMEDIATE.value, choice_values)
        self.assertIn(ExecutionMode.DEFERRED.value, choice_values)
