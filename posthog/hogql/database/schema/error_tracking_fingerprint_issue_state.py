...
# Column order used when building UNION-ALL override branches. Must cover every
# column referenced by the argmax subquery; order here is just for readability,
# the UNION matches by position so all branches must use the same order.
_OVERRIDE_COLUMNS: list[str] = [
    "team_id",
    "fingerprint",
    "issue_id",
    "issue_name",
...
def select_from_error_tracking_fingerprint_issue_state_table(
    ...
    overrides = _overrides_from_context(context)
    if overrides:
        # Replace the raw-table scan with `(base UNION ALL override1 UNION ALL ...) AS raw_table`
        # so argMax sees override rows with their (higher) versions and picks them.
        select.select_from = ast.JoinExpr(
            table=_build_union_with_overrides(overrides),
...
def _overrides_from_context(context: Optional[HogQLContext]) -> list[dict[str, Any]]:
    if context is None:
        return []
    return context.error_tracking_fingerprint_overrides or []


def _build_union_with_overrides(overrides: list[dict[str, Any]]):
    from posthog.hogql import ast

    base = ast.SelectQuery(
        select=[ast.Field(chain=[col]) for col in _OVERRIDE_COLUMNS],
        select_from=ast.JoinExpr(table=ast.Field(chain=[RAW_TABLE_NAME])),
    )

    override_selects: list[ast.SelectQuery | ast.SelectSetQuery] = [base]
    for o in overrides:
        override_selects.append(_override_select(o))

    return ast.SelectSetQuery.create_from_queries(override_selects, set_operator="UNION ALL")


def _override_select(row: dict[str, Any]):
    """Build a `SELECT <constants>` branch shaped to match the raw table column types.

    Casts match `ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_BASE_SQL` so UNION ALL
    type inference stays happy across branches.
    """
    from posthog.hogql import ast

    def _int64(value: Any):
        return ast.Call(name="toInt64", args=[ast.Constant(value=int(value))])

    def _int8(value: Any):
        return ast.Call(name="toInt8", args=[ast.Constant(value=int(value))])

    def _string(value: Any):
        return ast.Call(name="toString", args=[ast.Constant(value=str(value))])
    ...
    def _nullable_string(value: Any):
        if value is None:
            return ast.Call(name="CAST", args=[ast.Constant(value=None), ast.Constant(value="Nullable(String)")])
        return ast.Call(name="toNullable", args=[ast.Call(name="toString", args=[ast.Constant(value=str(value))])])

    def _nullable_int64(value: Any):
        if value is None:
            return ast.Call(name="CAST", args=[ast.Constant(value=None), ast.Constant(value="Nullable(Int64)")])
        return ast.Call(name="toNullable", args=[ast.Call(name="toInt64", args=[ast.Constant(value=int(value))])])

    def _nullable_uuid(value: Any):
        if value is None:
            return ast.Call(name="CAST", args=[ast.Constant(value=None), ast.Constant(value="Nullable(UUID)")])
        return ast.Call(name="toNullable", args=[ast.Call(name="toUUID", args=[ast.Constant(value=str(value))])])
    ...
    return ast.SelectQuery(
        select=[ast.Alias(alias=col, expr=column_exprs[col]) for col in _OVERRIDE_COLUMNS],
    )
