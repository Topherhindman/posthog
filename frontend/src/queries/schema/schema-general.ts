...
/**
 * Client-side override of a single denormalized fingerprint→issue state row. Used to hide
 * the Kafka→ClickHouse sync lag immediately after a mutation (assign/status/name/etc.). The
 * client sends one row per affected fingerprint; the backend UNIONs these into the argMax
 * subquery with the provided version so argMax picks them. `team_id` is stamped server-side.
 */
export interface ErrorTrackingFingerprintIssueStateOverride {
    fingerprint: string
    issue_id: string
...
export interface ErrorTrackingQuery extends DataNode<ErrorTrackingQueryResponse> {
    ...
    useQueryV3?: boolean
    /**
     * Client-side overrides to UNION into the denormalized fingerprint issue state subquery (V3 only).
