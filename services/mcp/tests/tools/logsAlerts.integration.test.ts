...
describe('Logs Alerts', { concurrent: false }, () => {
    ...
    describe('logs-sparkline-create', () => {
        const sparklineTool = GENERATED_TOOLS['logs-sparkline-create']!()

        function makeSparklineQuery(overrides: Record<string, unknown> = {}): Record<string, unknown> {
        ...
        it('should return sparkline data with date range', async () => {
            const result = await sparklineTool.handler(context, makeSparklineQuery())
            const data = parseToolResponse(result)

            expect(Array.isArray(data)).toBe(true)
        })

        it('should support severity breakdown', async () => {
            const result = await sparklineTool.handler(
                context,
                makeSparklineQuery({ sparklineBreakdownBy: 'severity' })
            )
            const data = parseToolResponse(result)
        ...
        it('should support service breakdown', async () => {
            const result = await sparklineTool.handler(context, makeSparklineQuery({ sparklineBreakdownBy: 'service' }))
            const data = parseToolResponse(result)

            expect(Array.isArray(data)).toBe(true)
        })

        it('should support severity level filter', async () => {
            const result = await sparklineTool.handler(
                context,
                makeSparklineQuery({ severityLevels: ['error', 'fatal'] })
            )
            const data = parseToolResponse(result)
